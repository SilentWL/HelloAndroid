...
public class BluetoothLeService extends Service {
    ...
    public void writeControlData(List<BluetoothGattService> gattServices, ArrayList<ArrayList<BluetoothGattCharacteristic>> GattCharacteristics,byte[] ControlData) {

        // Log.i("iDevice", iDevice);
        BluetoothGattService sendDataService = mBluetoothGatt
                .getService(gattServices.get(7).getUuid());
        if (sendDataService == null) {
            showMessage("send data service not found!");
            return;
        }
        // enableBattNoti(iDevice);
        BluetoothGattCharacteristic sendDataCharacteristic = GattCharacteristics.get(7).get(1);
        if (sendDataCharacteristic == null) {
            showMessage("send data charateristic not found!");
            return;
        }
        boolean status = false;

        Log.d(TAG, "WriteType=" + sendDataCharacteristic.getWriteType());


        sendDataCharacteristic.setValue(ControlData);
        //Log.e("发送的指令", "ControlData" + ControlData[0]);

        sendDataCharacteristic.setWriteType(BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE);
        status = mBluetoothGatt.writeCharacteristic(sendDataCharacteristic);
        Log.d(TAG, "writeControlData() - status=" + status);
    }

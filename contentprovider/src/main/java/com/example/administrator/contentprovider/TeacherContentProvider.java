...
public class TeacherContentProvider extends ContentProvider {
    ...
    public boolean onCreate() {
        ...
        dbOpenHelper = new DBOpenHelper(this.getContext(), ConstData.DATABASE_NAME, ConstData.DATABASE_VERSION);
        return true;
    }
    /**
    ...
    public Uri insert(Uri uri, ContentValues values){
        ...
        switch (ConstData.UserTableData.uriMatcher.match(uri)) {
            case ConstData.UserTableData.TEACHERS:
                id = db.insert("teacher", null, values);    // 返回的是记录的行号，主键为int，实际上就是主键值
                return ContentUris.withAppendedId(uri, id);
            case ConstData.UserTableData.TEACHER:
    ...
    public String getType(Uri uri) {
        switch (ConstData.UserTableData.uriMatcher.match(uri)) {
            case ConstData.UserTableData.TEACHERS:
                return ConstData.UserTableData.CONTENT_TYPE;
            case ConstData.UserTableData.TEACHER:
                return ConstData.UserTableData.CONTENT_TYPE_ITME;
            default:
                throw new IllegalArgumentException("Unknown URI " + uri);
        }
    }
